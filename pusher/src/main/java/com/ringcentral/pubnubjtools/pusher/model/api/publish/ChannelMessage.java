package com.ringcentral.pubnubjtools.pusher.model.api.publish;

import com.ringcentral.pubnubjtools.legacy.PubnubCrypto;
import com.ringcentral.pubnubjtools.pusher.RequestHelper;
import com.ringcentral.pubnubjtools.pusher.model.Credentials;
import com.ringcentral.pubnubjtools.pusher.model.EncryptionParameters;
...
public class ChannelMessage implements Message {
    ...
    private String buildPath() {
        String pubKey = PubnubCrypto.urlEncode(credentials.getPublisherKey());
        String subKey = PubnubCrypto.urlEncode(credentials.getSubscriberKey());
        String signature = "0";
        String channel = PubnubCrypto.urlEncode(this.channel);

        String payload = RequestHelper.toJson(serializedPayload);
        if (encryptionParameters.isPresent()) {
            payload = encrypt(payload, encryptionParameters.get());
        }
        payload = PubnubCrypto.urlEncode(payload);
